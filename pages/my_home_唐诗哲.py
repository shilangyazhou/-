...
def page_1():
    ...
    '''我的兴趣推荐'''
    st.write('### :blue[音乐推荐]')
    tab1, tab2, tab3, tab4 = st.tabs(["起风了", "告白气球", "求佛" ,"音乐软件推荐"])
    with tab1:
        st.image('起风了.jpg')  
        st.audio('起风了.mp3')      
    with tab2:  
    ...
    with tab3:
        ...
        st.audio('求佛.mp3')
    with tab4:
        go = st.selectbox('以下是我喜欢的音乐软件', ['网易云', 'QQ音乐'])
        if go == '网易云':
            st.link_button('前往网易云', 'https://music.163.com/')
        elif go == 'QQ音乐':
            st.link_button('前往QQ音乐', 'https://y.qq.com/')
